...
- Scripts should be idempotent by default — safe to re-run without side effects.
- Use environment variables for configuration with sensible defaults.
- Prefer portable POSIX-compatible constructs where possible; document any bash-specific features.
- Include a header comment block describing purpose, requirements, and environment variables.

## Code Style

- Use `log()` helper functions for user-facing output instead of raw `echo`.
...
- Scripts should support a `DRY_RUN` mode that prints actions without executing them.
